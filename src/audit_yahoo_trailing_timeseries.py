...
METRICS = ["trailingDilutedEPS", "trailingBasicEPS", "trailingNormalizedDilutedEPS"]
...
def _fetch_one(ticker: str, period1: int, period2: int) -> list[dict[str, object]]:
    ...
    try:
        ...
        print(f"VARNING {ticker}: trailing-timeseries kunde inte hämtas: {exc}")
        return []
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=max(1, min(workers, len(tickers)))) as executor:
        ...
        for index, future in enumerate(as_completed(futures), start=1):
            ...
            print(f"Yahoo trailing-timeseries {index}/{len(tickers)} klar: {ticker}")
    ...
    print(f"Yahoo trailing-timeseries: {len(result)} datapunkter för {result['ticker'].nunique() if not result.empty else 0}/{len(tickers)} tickers.")
    if not result.empty:
        ...
        print("Senaste Yahoo trailing-metrics för tidigare avvikelser:")
